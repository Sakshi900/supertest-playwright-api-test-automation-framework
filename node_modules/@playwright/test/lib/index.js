...
const playwrightFixtures = {
  ...
  _setupContextOptionsAndArtifacts: [async ({
    ...
    const stopTracing = async (tracing, contextTearDownStarted) => {
      ...
      if (captureTrace) {
        ...
        if (preserveTrace() || !contextTearDownStarted) {
          tracePath = path.join(_artifactsDir(), (0, _utils.createGuid)() + '.zip');
    ...
    const onWillCloseContext = async context => {
      ...
      await stopTracing(context.tracing, context[kStartedContextTearDown]);
    ...
    const onWillCloseRequestContext = async context => {
      const tracing = context._tracing;
      await stopTracing(tracing, context[kStartedContextTearDown]);
    };
