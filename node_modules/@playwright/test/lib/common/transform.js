...
function resolveHook(filename, specifier) {
  ...
  if (isRelativeSpecifier(specifier)) return (0, _util.js2ts)(_path.default.resolve(_path.default.dirname(filename), specifier));
  ...
  if (tsconfig && (isTypeScript || tsconfig.allowJs)) {
    ...
    for (const {
      ...
      for (const value of values) {
        ...
        const ts = (0, _util.js2ts)(candidate);
        if (ts) {
  ...
  return (0, _util.js2ts)(_path.default.resolve(_path.default.dirname(filename), specifier));
}
function transformHook(code, filename, moduleUrl) {
  ...
  try {
    ...
    const {
      code,
      map
    } = babelTransform(filename, isTypeScript, !!moduleUrl, hasPreprocessor ? scriptPreprocessor : undefined);
