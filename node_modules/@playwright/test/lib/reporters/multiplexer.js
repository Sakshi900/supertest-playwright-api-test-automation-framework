...
class Multiplexer {
  ...
  onError(error) {
    ...
    (0, _base.addSnippetToError)(this._config.config, error);
    for (const reporter of this._reporters) wrap(() => {
  ...
  onStepEnd(test, result, step) {
    ...
    for (const reporter of this._reporters) wrap(() => {
      ...
      return (_onStepEnd = (_ref2 = reporter).onStepEnd) === null || _onStepEnd === void 0 ? void 0 : _onStepEnd.call(_ref2, test, result, step);
    });
  }
  _addSnippetToTestErrors(test, result) {
    for (const error of result.errors) (0, _base.addSnippetToError)(this._config.config, error, test.location.file);
  }
  _addSnippetToStepError(test, step) {
    if (step.error) (0, _base.addSnippetToError)(this._config.config, step.error, test.location.file);
  }
}
